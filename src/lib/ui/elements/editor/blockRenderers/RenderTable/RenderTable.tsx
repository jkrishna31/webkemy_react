...
const RenderTable = ({ block, ...props }: any) => {
    const renderCell = (payload: any, header = false, key?: any) => {
        return header ? (
            <th className={`${styles.cell} ${header ? styles.header_cell : ""}`} key={"cell" + key}>
    ...
    const renderRow = (row: any, key?: any) => {
        return (
            <tr className={`${styles.row} ${row.isHeader ? styles.header_row : ""}`} key={"row" + key}>
                {
    ...
    return (
        <div data-block className={`${styles.table_wrapper} ${cStyles.block}`} id={block.id}>
            <table className={`${styles.data_table} ${styles.sticky_fr}`}>
